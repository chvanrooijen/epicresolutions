from django.shortcuts import render, redirect # Redirect is a new import to save the Roles and Causes in the session and then use these values to filter the Resolutions.
...
def select_cause(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('generate_resolutions')
    else:
        form = CauseForm()
    return render(request, 'resolutions/select_cause.html', {'form': form})

def generate_resolutions(request):
    ...
    return render(request, 'resolutions/resolutions.html', context)
